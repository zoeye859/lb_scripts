...
def find_region_name(dir_path):
    '''
    Got the name starting with 'ILTJ' from each selfcal direction folder
    Args:
        dir_path (str): the path to the directory in which a certain direction selfcal's results.
    Returns:
        file_name (str): the name starting with 'ILTJ'
    '''
...
def collect_png_byname(dir_path, cycle_num=10, png_dir_name = 'png_collection_by_name'):
    '''
    After running selfcal on multiple directions chosen from 6'' image, we gathered multiple folders.
    Each folder contains the selfcal results for one direction.
    We want to collect all the .png files from each of these folders to exam. The output files contain 
    the name of the .ms file
    Args:
        dir_path (str): the path to the directory in which the selfcal results are.
        png_dir_name (str): name of the png collection directory, default: 'png_collection_by_name'.
        cycle_num (int): maximum cycle number of the self-cal, default: 10
    Returns:
        png_output_path (str): the path to the png collection directory where we put all the .png files
    '''
